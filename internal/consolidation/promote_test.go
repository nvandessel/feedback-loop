...
func newTestPromoteConsolidator() *LLMConsolidator {
	return newTestLLMConsolidatorWithConfig(nil, LLMConsolidatorConfig{
		Model: "test-model",
	})
}
...
func TestLLMPromote_CreateNodes(t *testing.T) {
	...
	promoted, err := c.Promote(ctx, "test-run", memories, nil, nil, nil, s)
	if err != nil {
...
func TestLLMPromote_AbsorbMerge(t *testing.T) {
	...
	_, err := c.Promote(ctx, "test-run", []ClassifiedMemory{mem}, nil, merges, nil, s)
	if err != nil {
		t.Fatalf("Promote returned error: %v", err)
	}
...
func TestLLMPromote_MergeFailure(t *testing.T) {
	...
	promoted, err := c.Promote(ctx, "test-run", []ClassifiedMemory{mem}, nil, merges, nil, s)
	if err != nil {
		t.Fatalf("Promote returned error: %v", err)
	}
...
func TestLLMPromote_NilStore(t *testing.T) {
	...
	_, err := c.Promote(ctx, "test-run", []ClassifiedMemory{
...
func TestLLMPromote_DecisionLogging(t *testing.T) {
	...
	_, err := c.Promote(ctx, "test-run", []ClassifiedMemory{mem1, mem2}, nil, merges, nil, s)
	if err != nil {
		t.Fatalf("Promote returned error: %v", err)
	}
...
func TestLLMPromote_SupersedeRollbackOnUpdateFailure(t *testing.T) {
	...
	if _, err := c.Promote(ctx, "test-run", []ClassifiedMemory{mem}, nil, merges, nil, s); err != nil {
