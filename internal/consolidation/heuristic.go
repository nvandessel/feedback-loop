...
func (h *HeuristicConsolidator) Classify(ctx context.Context, candidates []Candidate) ([]ClassifiedMemory, error) {
	...
	for _, c := range candidates {
		...
		// Determine scope from project context
		scope := "universal"
		if projectID, ok := c.SessionContext["project_id"].(string); ok && projectID != "" {
...
func (h *HeuristicConsolidator) Promote(ctx context.Context, _ string, memories []ClassifiedMemory, edges []store.Edge, merges []MergeProposal, skips []int, s store.GraphStore) (int, error) {
	if s == nil {
		return 0, nil
	}
